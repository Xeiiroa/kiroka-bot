...
class Slash(commands.Cog):
    ...
    @app_commands.command(name="/Animeinfo", description="Give an anime title and get a description of the anime from the Anilist Api")
    async def animeinfo(self, ctx, *, Title: commands.StringOption(description="Title of an anime or manga")):
        
        info = self.get_info(Title)
        
        if info == None:
            await ctx.send(f"Anime {Title} was not found")
    ...
    #todo 
    #fix string for multiple die        
    #roll a die or multiple     
    @commands.slash_command(name="/roll", description="roll a given number of die and get back what each landed on")
    async def roll(self , ctx, die_number: commands.NumberOption(desription="Number of die you want to roll", default=1)): 
            if die_number > 1:
                rolls = "you rolled"
    ...
    @commands.slash_command(name="/between", description="Oyasu picks a number between 2 numbers you give")
    async def between(self, ctx, num1: commands.NumberOption(default=0), num2: commands.NumberOption(default=10)):
        z = random.randint(num1, num2)
        await ctx.send(z)
        
        
    #flip a coin
    @commands.slash_command(name="/flip", description="Oyasu flips a coin")
    ...
    @commands.slash_command(name="/choose", description="Oyasu chooses between 2 given option")
    ...
    async def lolstats(self, ctx, summonername: commands.StringOption(), region: commands.StringOption(choices=["br1","eun1","euw1","jp1","kr","la1","la2","na1","oc1","ph2","ru","sg2","th2","tr1","tw2","vn2"], default="na1")):
